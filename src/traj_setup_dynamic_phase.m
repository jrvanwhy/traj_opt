...
%         dx          The dynamics function (ẋ = f(x, u, a, n))
	...
	% The shared parameters are just the add_params specified by the user plus the duration; similarly,
	% the noopt_params are just the user-specified noopt params.
	phase.interval.shared_params = [{'duration'}; phase.names.add_params];
	...
	phase.interval.constraints{1} = traj_create_constraint('collocation', ...
		...
		phase.dynsys.dx((sym_start_params + sym_end_params)/2, sym_int_params, ...
			sym_shared_params(2:end), sym_noopt_params) * sym_shared_params(1) / phase.n_intervals);
	...
	for iter = 1:numel(phase.interval.constraints)
		...
		phase.interval.constraints{iter}.fcn = matlabFunction(phase.interval.constraints{iter}.fcn, 'vars', ...
			{sym_start_params, sym_end_params, sym_int_params, sym_shared_params, sym_noopt_params});
	end
	...
	syms sym_start_params sym_end_params sym_int_params sym_shared_params sym_noopt_params clear
end
