...
% x = [ position ]
%     [ velocity ]
% For testing purposes, the input is represented as a fourier series
%
...
function scenario = car_time(n_intervals, n_fourier)
	...
	add_params = {'dur', 'a0'};
	for iter = 1:n_fourier
		add_params{end+1} = ['a' num2str(iter)];
		add_params{end+1} = ['b' num2str(iter)];
	end
...
function [dx,min_force_con,max_force_con,inputfcn] = dynamic_system(x, a)
	...
	a0   = a(2);
	u    = a0/2;
	for iter = 1:((numel(a)-2)/2)
		u = u + a(2*iter+1)*cos(2*pi*iter*time/dur) + a(2*iter+2)*sin(2*pi*iter*time/dur);
	end
...
function [cost,start_state_con,end_state_con,dur_con,init_dur] = scenario_fcn(scenario)
	...
	% Constrain the additional duration parameter to equal the phase's duration
	% This is a small hack to deal with the lack of time variant dynamics support
	dur_con = traj_create_constraint('Additional duration', scenario.phases{1}.add_params(1), '=', scenario.phases{1}.duration);
	...
	% We need an additional initial duration constraint
	init_dur = traj_set_initval(scenario.phases{1}.add_params(1), 1);
end
